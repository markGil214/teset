...
const ARScannerPage: React.FC = () => {
  ...
  // Comprehensive reset function for all state and cleanup
  const resetAllState = useCallback(() => {
    console.log("=== RESET ALL STATE CALLED ===");
    ...
    baseScaleRef.current = 0.5;
    
    // Reset zoom controller
    if (zoomControllerRef.current) {
      console.log("Resetting zoom controller from", zoomControllerRef.current.getCurrentZoom(), "to 1.0x");
      zoomControllerRef.current.resetZoom();
      console.log("Zoom controller reset complete, new zoom:", zoomControllerRef.current.getCurrentZoom());
    } else {
      console.log("WARNING: Zoom controller not initialized when trying to reset");
    }
    
    console.log("=== ALL STATE RESET COMPLETED ===");
  }, [currentZoom, isZoomAnimating, showMaxZoomMessage, showSlicedModel, showConfirmation, modelLoading, modelError]);
  ...
  // Reset state when component unmounts or organ changes
  useEffect(() => {
    // Reset state when organ changes
    console.log(`=== ARScannerPage effect triggered for organ: ${organ.id} ===`);
    ...
    return () => {
      console.log("=== Component unmounting - performing final state reset ===");
      console.log("State before unmount cleanup:", {
        currentZoom,
        showSlicedModel,
        showConfirmation
      });
  ...
  }, [organ.id, resetAllState]); // Add organ.id to dependencies to reset when organ changes
  ...
  // Prevent body scrolling when AR is active and handle page unload
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      console.log("=== Page unloading - performing state reset ===");
      resetAllState();
    };

    window.addEventListener('beforeunload', handleBeforeUnload);

    // Cleanup: restore original overflow values and remove listener
    return () => {
  ...
  useEffect(() => {
    ...
    // Cleanup
    return () => {
      console.log("=== ARScannerPage cleanup started ===");
      
      // Reset all state before cleanup
      console.log("Performing state reset during cleanup");
      ...
      console.log("=== ARScannerPage cleanup completed ===");
    };
  }, [organ]);
  ...
  // Handle back to menu navigation with full state reset
  const handleBackToMenu = useCallback(() => {
    console.log("=== BACK TO MENU CLICKED ===");
    console.log("About to call resetAllState()");
    resetAllState();
    console.log("resetAllState() completed, now navigating to /scan-explore");
    // Navigate directly to scan-explore hub instead of using navigate(-1)
    navigate('/scan-explore', { replace: true });
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          🐛 Debug: Zoom={currentZoom.toFixed(1)}x | Sliced={showSlicedModel ? 'Yes' : 'No'} | Loading={modelLoading ? 'Yes' : 'No'}
        </div>
        {modelLoading && (
          <div
            style={{
