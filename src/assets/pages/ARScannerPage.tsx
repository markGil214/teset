...
const ARScannerPage: React.FC = () => {
  ...
  const [showMaxZoomMessage, setShowMaxZoomMessage] = useState(false);  const [zoomController] = useState(() => new ZoomController(1.0, {
    onZoomChange: (zoom: number) => {
      setCurrentZoom(zoom);
      setIsAnimating(false); // Animation completed
      // Apply zoom to the 3D model
      if (markerGroupRef.current) {
        markerGroupRef.current.scale.set(zoom, zoom, zoom);
      }
    },
    onMaxZoomReached: () => {
  ...
  useEffect(() => {
    ...
    scene.add(camera);      var markerGroup = new window.THREE.Group();
      scene.add(markerGroup);
      
      // Store markerGroup reference for zoom functionality
